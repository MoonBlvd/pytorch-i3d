...
class A3D(data_utl.Dataset):
    ...
    def make_dataset(self, split_file, split, root, mode):
        ...
        for vid in data.keys():
            # lis = pickle.load(open('/home/data/vision7/A3D_feat/A3D_utils/miss_0.pkl', 'rb'))
            # for vid in lis:
            ...
            # anomaly label
            labels[int(data[vid]['anomaly_class']), data[vid]['anomaly_start']:data[vid]
                   ['anomaly_end']] = 1    # binary classification
            # normal label
            labels[0, data[vid]['anomaly_end']:] = 1

            # for t in range(1, num_frames + 1,
            # (self.seq_len - self.overlap)):    # for A3D 1.0 which starts with 1
            for t in range(0, num_frames,
                           (self.seq_len - self.overlap)):    # for A3D 2.0 which starts with 0
